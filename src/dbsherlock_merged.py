    ...
    for i in range(len(all_causal_models)):
        ...
        for idx in merge_idx[1:]:
            ...
            for pred in preds:
                ...
                eps[pred] = p.predicate(
                    predicate_string, attr_num, attr_name, 0, type, lower, upper, c=0
                )  # attr_num, attr_name, type, a, b, c=0
...
for batch in range(batch_count):
    ...
    ###################################################################
    merged_causal_models = merge(
        all_causal_models, train_sample
    )  # 각 배치마다 merged causal model을 만들기 1*10
    if batch == 0:
        with open(
            "merged/merged_causal_models_{}.txt".format(num_train_sample), "wb"
        ) as fa:
    ...
    for i in range(num_case):
        for j in test_sample:
            ...
            if save:
                with open(
                    "merged/explanation/{}_{}_{}_{}.txt".format(
                        num_train_sample, batch, i, j
                    ),
                    "wb",
                ) as fe:
